...
class Bottlenecka(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.cv1 = Convv(c1, c_, 1, 1)
        self.cv2 = Convv(c_, c2, 3, 1, g=g)
...
class BottleneckCSPa(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv4 = Convv(2 * c_, c2, 1, 1)
        self.bn = nn.BatchNorm2d(2 * c_)  # applied to cat(cv2, cv3)
        self.act = nn.LeakyReLU(0.1, inplace=True)
        self.m = nn.Sequential(*[Bottlenecka(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
...
class CSPBottleneck3Conva(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(CSPBottleneck3Conva, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Convv(c1, c_, 1, 1)
        self.cv2 = Convv(c1, c_, 1, 1)
        self.cv3 = Convv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*[Bottlenecka(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
...
class SPP2(nn.Module):
    ...
    def __init__(self, c1, c2, k=(5, 9, 13)):
        ...
        self.cv1 = Convv(c1, c_, 1, 1)
        self.cv2 = Convv(c_ * (len(k) + 1), c2, 1, 1)
...
class Focus1(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = Convv(c1 * 4, c2, k, s, p, g, act)
